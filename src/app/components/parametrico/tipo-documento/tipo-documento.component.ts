...
import { PopUpGenericoService, MensajesService, TipoDocumentoService } from '../../../services/service.index';
import { FormBuilder } from '@angular/forms';
import { merge, of } from 'rxjs';
import { startWith, switchMap, map, catchError } from 'rxjs/operators';
import Swal from 'sweetalert2';
import { RolesEditComponent } from '../../seguridad/roles/roles-edit.component';
...
@BlockUI() blockUI: NgBlockUI;
  ...
  acciones(data,e){
    ...
    switch (actionType) {
      case 'activar':
        Swal.fire({
          title: 'ACTIVAR',
          text:  MENSAJES_DIALOG.ACTIVAR + ' el tipo documento: ' + data.nombre +' ?',
          icon: 'warning',
          ...
          if (result.value) {
            ...
            this.blockUI.start()
            this.service.activarRecurso(data.idTipoDocumento).subscribe( (r:any) =>{
      ...
      case 'desactivar':
        Swal.fire({
          title: 'DESACTIVAR',
          text:  MENSAJES_DIALOG.DESACTIVAR + ' el tipo documento: ' + data.nombre +' ?',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonText: 'Confirmar',
          cancelButtonText: 'Cancelar',
          ...
          if (result.value) {
            ...
            this.blockUI.start()
            this.service.desactivarRecurso(data.idTipoDocumento).subscribe( (r:any) =>{
      ...
      case 'editar':
        ...
        'Aceptar', 'Cancelar','sm',data,RolesEditComponent).then((dataResult:any)=>
        {
          if(dataResult){
           this.blockUI.start()
           this.service.modificarRecurso( dataResult,data.idTipoDocumento).subscribe( (r:any) =>{
  ...
  agregar(){
    this.popUpService.confirm('Agregar Tipo Documento', '',
    'Aceptar', 'Cancelar','sm',null,TipoDocumentoEditComponent).then((dataResult:any)=>
    {
      if(dataResult){
       dataResult.activo='S';
       this.blockUI.start()
